...
# Application Header with Rebranded Logo
import os
...
# 1. Try exact match relative to script or working directory
for base_path in [script_dir, os.getcwd()]:
    exact_path = os.path.join(base_path, "shassee_tech_final_logo.png")
    if os.path.exists(exact_path):
        logo_path = exact_path
        logo_exists = True
        break

# 2. Try case-insensitive search in both directories for any file containing 'logo'
if not logo_exists:
    for base_path in [script_dir, os.getcwd()]:
        try:
            for file in os.listdir(base_path):
                if "logo" in file.lower() and file.lower().endswith(('.png', '.jpg', '.jpeg')):
                    logo_path = os.path.join(base_path, file)
                    logo_exists = True
                    break
            if logo_exists:
                break
...
# Render Header Logo or Fallback Text
if logo_exists:
