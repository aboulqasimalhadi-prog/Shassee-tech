...
st.set_page_config(
    page_title="شاصي تك | SHASSEE TECH v27",
    page_icon="⚙️",
    layout="wide",
...
if silver_car_b64:
    ...
    # High-quality elegant SVG fallback representing a detailed vehicle chassis frame (completely safe from blank line collapses)
        # Elegant, clean, minimalist neon typographic design panel (completely avoids any funny ladder or robot drawings)
    silver_car_html = '''
...
st.sidebar.markdown("<hr style='border-color: rgba(255,255,255,0.05);'>", unsafe_allow_html=True)
st.sidebar.subheader("🔌 ربط البيانات وتخطي حظر المزادات")

bidcars_key = st.sidebar.text_input(
    "🔑 مفتاح ترخيص bid.cars API (تجاري):", 
    type="password", 
    value=st.session_state.get("bidcars_key", ""),
    help="أدخل مفتاح ترخيص API التجاري من bid.cars لسحب البيانات الحية والصور الحقيقية بدون حظر!"
)
st.session_state["bidcars_key"] = bidcars_key

scrapingbee_key = st.sidebar.text_input(
    "🔑 مفتاح ScrapingBee API (مجاني):", 
    type="password", 
    value=st.session_state.get("scrapingbee_key", ""),
    help="للحصول على مفتاح مجاني يمنحك 1,000 طلب شهرياً لتخطي الحماية جرب: scrapingbee.com"
)
st.session_state["scrapingbee_key"] = scrapingbee_key

apibara_key = st.sidebar.text_input(
    "🔑 مفتاح Apibara API (تجريبي):", 
    type="password", 
    value=st.session_state.get("apibara_key", ""),
    help="أدخل مفتاحك التجريبي من apibara.tech (إن وجد) لسحب بيانات السيارات."
)
st.session_state["apibara_key"] = apibara_key

if bidcars_key.strip() or scrapingbee_key.strip() or apibara_key.strip():
...
def parse_auction_url(url):
    """
    Parses a Copart or IAAI URL using multi-layered API or scraping bypass.
    Attempts bid.cars API, ScrapingBee API, cloudscraper, and falls back to an intelligent simulation.
    """
    ...
    # Default fallback values (which will be overwritten if API/scraper succeeds)
    year = 2022
    make = "Toyota"
    model = "Toyota Tacoma"
    import hashlib
    ...
    # --- LAYER 1: Commercial bid.cars API (Bypasses Cloudflare cleanly with commercial premium database) ---
    bidcars_key = st.session_state.get("bidcars_key", "")
    if bidcars_key.strip():
        try:
            response = requests.get(
                f"https://api.bid.cars/v1/lots/{lot_id_num}",
                params={"api_key": bidcars_key},
                timeout=8
            )
            if response.status_code == 200:
                api_response = response.json()
                api_data = api_response.get("data", {}) if "data" in api_response else api_response
                vin = api_data.get("vin", vin)
                make = api_data.get("make", make)
                model = f"{make} {api_data.get('model', '')}" if api_data.get('model') else model
                year = int(float(api_data.get("year", year)))
                platform = api_data.get("auction", platform).capitalize()
                location = api_data.get("location", location)
                title = api_data.get("doc_type", title)
                current_bid = float(api_data.get("current_bid", current_bid))
                impact = api_data.get("damage", impact)
                
                images = api_data.get("images", [])
                if images:
                    images_scraped = [img.get("url") for img in images if isinstance(img, dict) and img.get("url")]
                    st.session_state[f"images_{lot_id}"] = images_scraped
                    
                api_success = True
                st.toast("🎉 نجاح حقيقي! تم استرداد البيانات الفعلية حياً من المزاد عبر bid.cars API!")
            elif response.status_code == 401:
                st.sidebar.error("❌ مفتاح ترخيص bid.cars غير صالح أو غير نشط!")
            else:
                st.sidebar.warning(f"⚠️ استجابة غير متوقعة من المزود: Code {response.status_code}")
        except Exception:
            pass

    # --- LAYER 2: ScrapingBee API (Bypasses Cloudflare using headless browser proxies) ---
    scrapingbee_key = st.session_state.get("scrapingbee_key", "")
    if not api_success and scrapingbee_key.strip():
        try:
            response = requests.get(
                "https://app.scrapingbee.com/api/v1/",
                params={"api_key": scrapingbee_key, "url": url_cleaned, "render_js": "false"},
                timeout=12
            )
            if response.status_code == 200:
                ...
                # Parse Copart or IAAI page structure
                if platform == "Copart":
                    # Title
                    ...
                    if title_tag:
                        ...
                        if len(words) >= 2:
                            ...
                            model = " ".join(words[1:])
                    # VIN
                    vin_tag = soup.select_one("div[data-uname='vinValue']") or soup.select_one("span[data-uname='vinValue']")
                    if vin_tag:
                        vin = vin_tag.text.strip()
                    # Location
                    ...
                    if title_tag2:
                        title = title_tag2.text.strip()
                    # Current Bid
                    bid_tag = soup.select_one("span[data-uname='bidPrice']")
                    if bid_tag:
                        try:
                            current_bid = float(bid_tag.text.strip().replace('$', '').replace(',', ''))
                        except:
                            pass
                else:
                    # Basic IAAI Parser fallback
                    title_tag = soup.select_one("h1.lot-title")
                    if title_tag:
                        title_text = title_tag.text.strip()
                        year_match = re.search(r'\b(20[0-2][0-9]|199[0-9])\b', title_text)
                        year = int(year_match.group(1)) if year_match else year
                ...
                api_success = True
                st.toast("🎉 نجاح حقيقي! تم تخطي حماية المزاد وسحب البيانات الحية عبر ScrapingBee API!")
        except Exception:
            pass

    # --- LAYER 3: Apibara API (Developer fallback) ---
    apibara_key = st.session_state.get("apibara_key", "")
    if not api_success and apibara_key.strip():
        try:
            response = requests.get(
            ...
            if response.status_code == 200:
                ...
                api_success = True
                st.toast("🎉 نجاح! تم سحب البيانات الفعلية حياً عبر Apibara API!")
        except Exception:
            pass

    # --- LAYER 4: Cloudscraper scraping directly (Free with high risk of 403 blocks) ---
    if not api_success:
        try:
            scraper = cloudscraper.create_scraper()
            response = scraper.get(url_cleaned, timeout=10)
            if response.status_code == 200:
                ...
                if title_tag:
                    ...
                    api_success = True
                    st.toast("🎉 نجاح حقيقي! تم سحب البيانات المباشرة بدون مفاتيح عبر cloudscraper bypass!")
        except Exception:
            pass

    # --- LAYER 5: Smart Guess fallback if all else failed or blocked ---
    if not api_success:
