...
st.set_page_config(
    page_title="شاصي تك | SHASSEE TECH v14",
    page_icon="🚗",
    layout="wide",
...
with tab1:
    ...
    with col_scrape_1:
        scraped_url_input = st.text_input("🔗 ألصق رابط سيارة مباشر هنا (مثال: https://www.copart.com/lot/12345678/2022-toyota-tacoma...):", placeholder="أدخل الرابط...")
    with col_search_btn:
        st.markdown("<div style='padding-top: 28px;'></div>", unsafe_allow_html=True)
        if st.button("⚡ ربط وسحب السيارة مجاناً", key="btn_trigger_scrape"):
            if scraped_url_input:
                with st.spinner("⏳ جاري فك شيفرة الرابط وتخطي قيود الحماية والـ API..."):
                    ...
                    # Auto-select the newly added or existing car and force load it immediately!
                    st.session_state["selected_car_id"] = new_lot["id"]
                    st.success(f"🎉 نجاح خارق! تم سحب وتنشيط سيارة لوت {new_lot['id']} وعرضها تلقائياً بالتقرير والـ 3D Heatmap في الأسفل!")
                    st.rerun()
            else:
